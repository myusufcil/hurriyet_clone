...
class MainListViewHolder(var view: View) : BaseViewHolder(view) {
    ...
    var item_spot_image_spot_activity = view.findViewById(R.id.image) as ImageView
    var item_spot_text = view.findViewById<TextView>(R.id.title)

    override fun bindView(baseModel: BaseModel, position: Int) {
        val item = baseModel as MainListDTO
        Glide.with(view.context).load(item.imageUrl).apply(RequestOptions().centerCrop())
